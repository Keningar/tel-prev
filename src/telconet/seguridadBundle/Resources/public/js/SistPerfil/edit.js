...
function actualizarAsignacion(strAccion)
{ 
    var objSelector = null;
   
    if( strAccion == 'Eliminar')
    {
        objSelector = sm2.getSelection();
    }
    else
    {
        objSelector = sm.getSelection();
    }
    
    if(objSelector.length > 0)
    {  
        var arrayAsignaciones = new Object();
        var arrayData         = new Array();
        var boolContinuar     = true;
        
        arrayAsignaciones['asignaciones'] = new Array();
        ...
        for(var i=0 ;  i < objSelector.length ; ++i)
        {
            ...
            {
                arrayData.push({ 
                                    modulo_id: objSelector[i].get('modulo_id'), 
                                    accion_id: objSelector[i].get('accion_id')
                              });
            }
            else
            {
                var r = Ext.create('Asignacion', {
                                                    perfil_id:     '',
                                                    modulo_id:     objSelector[i].get('modulo_id'),
                                                    modulo_nombre: objSelector[i].get('modulo_nombre'),
                                                    accion_id:     objSelector[i].get('accion_id'),
                                                    accion_nombre: objSelector[i].get('accion_nombre')
                                                  }
                                  );   
        ...
        if( boolContinuar )
        {
            ...
            {
                arrayAsignaciones['asignaciones'] = arrayData;

                Ext.Ajax.request
                ({
                    method: 'post',
                    url: strUrlUpdatePerfil,
                    params: 
                    {
                        data: Ext.JSON.encode(arrayAsignaciones),
                        accion: strAccion
                    },
                    success: function(response)
                    {
                        ...
                        if( text == 'OK' )
                        {
                            ...
                            Ext.Msg.alert('Atencion', 'Sus cambios han sido guardados con éxito');
                        }
                        else
                        {
                            Ext.Msg.alert('Error', 'Hubo un problema al agregar los perfiles');
                        }
                    },
            ...
            {
                Ext.Msg.alert('Atención', 'Debe seleccionar al menos una acción');
            }
        }  
    }
    else
...
function existeAsignacion(myRecord, grid)
{
    ...
    for(var i=0; i < num ; i++)
    {
        ...
        { 
            existe = true;
            alert(  'Ya existe una asignacion similar que contiene el Modulo '+myRecord.get('modulo_nombre')+' y la Accion '
                    +myRecord.get('accion_nombre') );
