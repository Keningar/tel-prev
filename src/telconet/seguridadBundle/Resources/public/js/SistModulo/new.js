 ...
 */
//se agrega validacion de nombreModulo - debe ser unico
...
function validarFormulario()
{
    ...
    //se agrega validacion de nombreModulo - debe ser unico
    Ext.Ajax.request({
        url: "validaUnicoModuloAjax",
        method: 'post',
        params: {nombreModulo: document.getElementById('telconet_seguridadBundle_sistmodulotype_nombreModulo').value},
        success: function(response) {
            var text = response.responseText;

            text = text.toString();
          
            var n = text.indexOf("existe");

            if (n > 0)
            {
                Ext.Msg.alert('Mensaje', 'Ya existen modulos creados con este nombre');
            }
            else
            {
        ...
        {
            Ext.Msg.alert('Mensaje', 'Error al validar Modulo '.result.responseText);
        }
    });
