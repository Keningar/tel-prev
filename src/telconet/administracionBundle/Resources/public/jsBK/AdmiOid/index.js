Ext.onReady(function() {    
    ...
    var storeMarcaElemento = new Ext.data.Store({ 
        ...
        proxy: {
            type: 'ajax',
            url : '../../tecnico/admi_marcas_elementos/getMarcasElementos',
            reader: {
                type: 'json',
                totalProperty: 'total',
                root: 'encontrados'
...
function buscar(){
    store.getProxy().extraParams.tipoMedio = Ext.getCmp('sltMarcaElemento').value;
...
function eliminarAlgunos(){
    ...
    if(sm.getSelection().length > 0)
    {
      ...
      {
        param = param + sm.getSelection()[i].data.idOid;
