Ext.onReady(function() {    
    ...
    var storeBuffer = new Ext.data.Store({ 
        total: 'total',
        autoLoad:true,
        proxy: {
            type: 'ajax',
            url : '../../tecnico/admi_buffer/getBufferes',
            reader: {
                type: 'json',
                totalProperty: 'total',
                root: 'encontrados'
                ...
                },
                {
                    ...
                    items: [{
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            window.location = ""+rec.get('idHilo')+"/show";
                            }
                        },
                        {
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            var rec = store.getAt(rowIndex);
                            if(rec.get('action2')!="button-grid-invisible")
                                window.location = ""+rec.get('idHilo')+"/edit";
...
function buscar(){
    store.getProxy().extraParams.tipoMedio = Ext.getCmp('sltBuffer').value;
...
function limpiar(){
    ...
    Ext.getCmp('sltBuffer').value="";
    Ext.getCmp('sltBuffer').setRawValue("");
...
function eliminarAlgunos(){
    ...
    if(sm.getSelection().length > 0)
    {
      ...
      {
        param = param + sm.getSelection()[i].data.idHilo;
