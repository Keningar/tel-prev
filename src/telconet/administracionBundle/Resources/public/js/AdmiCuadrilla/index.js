...
{ 
    ...
    if(boolPermiso1 && boolPermiso2)
    {
        selModel = Ext.create('Ext.selection.CheckboxModel', 
        {
            ...
            {
                if (record.get('strEstado') == 'Activo' || record.get('strEstado') == 'Modificado')
                {
    ...
    grid = Ext.create('Ext.grid.Panel', 
    {
        ...
        [
            ...
            {
              header: "<p style='text-align:center;line-height:15px;'>Libre</p>",
            ...
            {
              header: "<p style='text-align:center;line-height:15px;'>Hora<br/>Inicio</p>",
              dataIndex: 'strTurnoInicio',
              width: 50,
              sortable: true
            },
            {
              header: "<p style='text-align:center;line-height:15px;'>Hora<br/>Fin</p>",
            ...
            {
                xtype: 'actioncolumn',
                header: "<p style='text-align:center;line-height:15px;'>Acciones</p>",
                width: 200,
                items: 
                [
                    {
                    ...
                    },
                    {
                        getClass: function(v, meta, rec) 
                        {
                            ...
                            {
                                this.items[2].tooltip = 'Eliminar Cuadrilla';
                            }
                    ...
                    },
                    {
                        getClass: function(v, meta, rec) 
                        {
                            ...
                            if( rec.get('strEstado') != "Es Prestamo" ) 
                            {
                                strClassButton        = '';
                            ...
                            {
                                if( rec.get('coordinadorPrincipalId') != rec.get('coordinadorPrestadoId') )
                                {
                                    strClassButton        = '';
                                    this.items[4].tooltip = '';
                                }
                                ...
                                {
                                    this.items[4].tooltip = 'Devolver Cuadrilla';
                                }
                            }

                            return strClassButton;
                        },
                        tooltip: 'Devolver Cuadrilla',
                        handler: function(grid, rowIndex, colIndex) 
                        {
                            var rec = store.getAt(rowIndex);

                            if(rec.get('strEstado') == "Es Prestamo" && rec.get('coordinadorPrincipalId') == rec.get('coordinadorPrestadoId'))
                            {
                                var arrayParametros                        = [];
                                    arrayParametros['cuadrillas']           = rec.get('intIdCuadrilla');
                    ...
                    
                    {
                        getClass: function(v, meta, rec) 
                        {
                            ...
                            {
                                if((rec.get('strEstado')=='Activo' || rec.get('strEstado')=='Es Prestamo') && rec.get('strEstaLibre')!='SI')
                                {
                    ...
                    },
                    {
                        getClass: function(v, meta, rec) 
                        {
                            var strClassButton = 'button-grid-reactivarCuadrillaLibre';
                            var permiso = $("#ROLE_170-4977");
                            var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);
                            if(!boolPermiso){ strClassButton = ""; }
                            ...
                            else
                            {
                                if((rec.get('strEstado')=='Activo' || rec.get('strEstado')=='Es Prestamo') && rec.get('strEstaLibre')=='SI')
                                {
                                    this.items[6].tooltip = 'Dejar Cuadrilla Operativa';
                                }
                                else
                                {
                                    strClassButton          = "";
                        ...
                        tooltip: 'Dejar Cuadrilla Operativa',
                        handler: function(grid, rowIndex, colIndex) 
                        {
                            var strClassButton = 'button-grid-reactivarCuadrillaLibre';
                            var permiso = $("#ROLE_170-4977");
                            ...
                            if (strClassButton != "")
                            {
                                ...
                                if((rec.get('strEstado')=='Activo' || rec.get('strEstado')=='Es Prestamo') && rec.get('strEstaLibre')=='SI')
                                {
                                    ...
                                    arrayParametros['intIdCuadrilla']                       = rec.get('intIdCuadrilla');
                                    arrayParametros['strNombreCuadrilla']                   = rec.get('strNombreCuadrilla');
                                    arrayParametros['strEstadoAccionARealizarCuadrilla']    = 'OPERATIVA';
                                    administrarCuadrillaLibre(arrayParametros);
                                }
                            }
                        }
        ...
        ({
            ...
            listeners: 
            {
                load: function(store, records)
                {
                     ...
                     store.insert(1, [{ strNombre: 'Es prestamo', strValue: 'Es_Prestamo' } ]);
                }      
            }
        });
        ...
        if(btn=='yes')
        {
            ...
            ({
                ...
                success: function(result)
                {
                    ...
                    {
                        Ext.Msg.alert('Información', 'Cuadrilla eliminada con éxito');
                    }
                    else
                    {
...
function eliminarAlgunos()
{
    ...
    if(selection.length > 0)
    {
        ...
        {
            if( selection[i]['data'].strEstado != 'Eliminado' && selection[i]['data'].strEstado != 'Prestado'
                && selection[i]['data'].strEstado != 'Es Prestamo' )
            {
        ...
        {
            var arrayParametros               = [];
                arrayParametros['cuadrillas'] = param;

            eliminarAccionAjax(arrayParametros);
        }
        else
        {
            Ext.Msg.alert('Error ','Debe seleccionar registros válidos para eliminar');
        } 
    }
    else
    {
            ...
            [
                {
                    ...
                    handler: function()
                    {
                        ...
                        if( form.isValid() )
                        {
                            ...
                            var comboDiaSemana1  = Ext.getCmp('comboDiaSemana1').getValue();
...
function eliminarAsignacionVehicular(arrayParametros)
{
    var strMensaje = 'Se eliminará la asignación del vehículo con placa '+arrayParametros['placa'];
        strMensaje+= ' y la cuadrilla '+arrayParametros["nombreCuadrilla"];
    ...
    Ext.Msg.confirm('Alerta',strMensaje, function(btn)
    {
        if(btn=='yes')
        {
            connEsperaAccion.request
            ({
                url: strUrlEliminarAsignacionVehicular,
                method: 'post',
                dataType: 'json',
                params:
                { 
                    idElemento: arrayParametros['elemento'] ,
                    idCuadrilla:arrayParametros['cuadrilla']
                ...
                success: function(result)
                {
                    ...
                    if( strResult=="OK" )
                    {
                        var strMensajeEliminarAsignacion = 'Se eliminó la asociación entre el vehículo con placa '+arrayParametros['placa'];
                            strMensajeEliminarAsignacion+= ' y la cuadrilla  '+arrayParametros["nombreCuadrilla"]+' ';
...
function validarHoras(cmp, arrayParametros)
{
    ...
    if(valueFechaDesdeAsignacion && valueFechaHastaAsignacion)
    {
        ...
        {
            boolOKFechas=false;
            strMensaje='La Fecha Inicio '+ formattedValueFechaDesdeAsignacion +' no puede ser mayor a la Fecha Fin '+formattedValueFechaHastaAsignacion;
    ...
    {
        //const date = new Date('29-03-2023'+' '+'07:00');
        let diaFechaInicio = valueFechaDesdeAsignacion.getDate();
        let mesFechaInicio = valueFechaDesdeAsignacion.getMonth();
        let AnoFechaInicio = valueFechaDesdeAsignacion.getFullYear();
        /*let horasFechaInicio = valueFechaDesdeAsignacion.getFullYear();
        let minutosFechaInicio = valueFechaDesdeAsignacion.getMonth();*/

        let diaFechaFin = valueFechaDesdeAsignacion.getDate();
        let mesFechaFin = valueFechaDesdeAsignacion.getMonth();
        let AnoFechaFin = valueFechaDesdeAsignacion.getFullYear();
        /*let horasFechaFin = valueFechaDesdeAsignacion.getMonth();
        let minutosFechaFin = valueFechaDesdeAsignacion.getFullYear();*/

        valueHoraDesdeAsignacion.setFullYear(AnoFechaInicio);
        valueHoraDesdeAsignacion.setMonth(mesFechaInicio);
        valueHoraDesdeAsignacion.setDate(diaFechaInicio);

        valueHoraHastaAsignacion.setFullYear(AnoFechaFin);
        valueHoraHastaAsignacion.setMonth(mesFechaFin);
        valueHoraHastaAsignacion.setDate(diaFechaFin);
        ...
        {
            // se debe sumar un dia de la fecha hasta para evaluar que las fecha desde sea mayor a la fecha
            boolOKHoras=false;
    ...
    if( boolOKHoras && boolCamposLLenos && boolOKFechas)
    {
        return true;
    }
    else
...
{
    Ext.getCmp('strFechaInicio').enable();
    Ext.getCmp('strFechaFin').enable();
    Ext.getCmp('strHoraInicio').enable(); 
