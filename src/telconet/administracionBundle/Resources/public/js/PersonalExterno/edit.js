...
Ext.onReady(function()
{
    Ext.create('Ext.panel.Panel',
        {
            ...
            border: false,
            buttonAlign: 'center',
            renderTo: 'form_personal_externo_update',
            layout:
                {
                    type: 'vbox'
                },
                ...
                [
                    {
                        ...
                        handler: function()
                        {
                            if (validarFormasContacto($('#personalexternotype_formas_contacto')))
                            {
                                ...
                                connEsperaAccion.request(
                                    {
                                        url: urlActualizarPersonalExterno,
                                        method: 'POST',
                                        timeout: 60000,
                                        params:
                                            {
                                                ...
                                                strNombres: $('#personalexternotype_nombres').val(),
                                                strApellidos: $('#personalexternotype_apellidos').val(),
    ...
    getEmpresasExternas(intIdPersonaEmpresaRol);
    ...
    if( strPrefijoEmpresa == "TN" )
    {
        ...
        ({
            ...
            listeners: 
            {
                ...
                {
                    if( !Ext.isEmpty(intIdDepartamentoEmpresaSession) )
                    {
                        Ext.each(records, function(record)
                        {
                            if( record.get('id_departamento') == intIdDepartamentoEmpresaSession )
                            {
                                Ext.getCmp('cmbDepartamentoEmpresaSession').setValue(record.get('id_departamento'));
                            }
                        });
                    }
                }      
    ...
    if ($('#Estado').val() == 'Eliminado')
    {
        ...
        Ext.Msg.show(
            {
                title: 'Error',
                msg: 'No se puede editar el Personal Externo <br>\"' +
                    $('#personalexternotype_identificacionCliente').val() + '-' +
                    $('#personalexternotype_apellidos').val() + ' ' +
                    $('#personalexternotype_nombres').val() +
                ...
                success: function(parResponse)
                {
                    ...
                    if (parsedJSON.msg == '')
                    {
                        ...
                        if (results !== false)
                        {
                            ...
                            var personal_externo_id = (resultPersExt.id ? parseInt(resultPersExt.id * 1) : "");
                            ...
                            $('#personalexternotype_nombres').val(nombres);
                            $('#personalexternotype_apellidos').val(apellidos);
