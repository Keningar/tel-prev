...
function validarDocumentosObligatoriosPorPersona()
{
    var tiposDocs = document.getElementById("imagenes-fields-list").getElementsByTagName("li");
    var idPersonaEmpresaRol = document.getElementById("idPersonaEmpresaRol").value;
    ...
    else 
    {
        if(contArchivosVacíos>0)
        {
            ...
            Ext.Msg.confirm('Alerta','Hay archivos sin seleccionar. Desea subir los archivos restantes?', function(btn)
            {
                if(btn=='yes')
                {
                    enviarAGuardarArchivos(strIdTiposDocsPorSubir,idPersonaEmpresaRol,strFechasHastaDocsPorSubir);
                }
            });
        }
        else
...
function enviarAGuardarArchivos(strIdTiposDocsPorSubir,idPersonaEmpresaRol,strFechasHastaDocsPorSubir)
{
