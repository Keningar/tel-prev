...
Ext.onReady(function() { 
    ...
    var storeConector = new Ext.data.Store({ 
        total: 'total',
        autoLoad:true,
        proxy: {
            type: 'ajax',
            url : '../../tecnico/admi_conector_interface/getConectoresInterfaces',
            reader: {
                type: 'json',
                totalProperty: 'total',
                root: 'encontrados'
        ...
        fields:
              [
                {name:'nombreConectorInterface', mapping:'nombreConectorInterface'},
                {name:'idConectorInterface', mapping:'idConectorInterface'}
              ]
    });
    ...
    Ext.define('ModelStore', {
		...
		[				
			...
			{name:'conectorInterface', mapping:'conectorInterface'},
			{name:'estado', mapping:'estado'},
			{name:'action1', mapping:'action1'},
			{name:'action2', mapping:'action2'},
			{name:'action3', mapping:'action3'}                
		],
        idProperty: 'idTipoInterface'
    });
	...
	//****************  DOCKED ITEMS - BOTONES PARTE SUPERIOR ************************
        var permiso = '{{ is_granted("ROLE_129-8") }}';
        var boolPermiso1 = (typeof permiso === 'undefined') ? false : (permiso ? true : false);	
	
        var permiso = '{{ is_granted("ROLE_129-9") }}';
        ...
        columns:[
                {
                  ...
                  dataIndex: 'idTipoInterface',
                  hidden: true,
                  hideable: false
                },
                {
                  id: 'nombreTipoInterface',
                  header: 'Nombre Tipo Interface',
                  dataIndex: 'nombreTipoInterface',
                  width: 200,
                  sortable: true
                },
                {
                  id: 'conectorInterface',
                  header: 'Conector Interface',
                  dataIndex: 'conectorInterface',
                  width: 150,
                  sortable: true
                },
                {
                ...
                },
                {
                    ...
                    items: [
						{
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_129-6") }}';
				    ...
				    var permiso = '{{ is_granted("ROLE_129-6") }}';
								...
								if(rec.get('action1')!="icon-invisible")
									window.location = rec.get('idTipoInterface')+"/show";
								else
                        ...
                        {
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_129-4") }}';
                                    var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso ? true : false);
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
	                            ...
	                            if(rec.get('action2')!="icon-invisible")
	                                window.location = rec.get('idTipoInterface')+"/edit";
								else
									Ext.Msg.alert('Error ','No tiene permisos para realizar esta accion');
							}
                        },
	                                ...
	                                Ext.Msg.confirm('Alerta','Se eliminara el registro. Desea continuar?', function(btn){
	                                    if(btn=='yes'){
	                                        Ext.Ajax.request({
	                                            url: "deleteAjax",
	                                            method: 'post',
	                                            params: { param : rec.get('idTipoInterface')},
        ...
        title: 'Criterios de busqueda',
            buttons: [
                ...
                items: [
                        ...
                        {
                            xtype: 'combo',
                            id: 'sltConectorInterface',
                            fieldLabel: 'Conector Interface',
                            store: storeConector,
                            displayField: 'nombreConectorInterface',
                            valueField: 'idConectorInterface',
                            width: '30%'
                        },
...
function buscar(){
    store.getProxy().extraParams.conectorInterface = Ext.getCmp('sltConectorInterface').value;
...
function limpiar(){
    ...
    Ext.getCmp('sltConectorInterface').value="";
    Ext.getCmp('sltConectorInterface').setRawValue("");
