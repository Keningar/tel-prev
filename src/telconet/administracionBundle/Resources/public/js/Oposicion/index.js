...
//validaIdentificacion(isValidarIdentificacionTipo);
/**
...
function CreaGridPoliticasClausula(currenIdentificacion){
    ...
    let strParametros = JSON.stringify({ identificacion: currenIdentificacion});
     let storeRespuestaAnterior = Ext.create('Ext.data.Store', {
                        ...
                        columns: [
                            ...
                            },
                            {
                                ...
                                renderer: function (
                                    ...
                                    html += '<select name="valorRespuestaCmb-'+indice+'" style="width:50px" id="valorRespuestaCmb-'+indice+'">';
                                        ...
                                        clausulas.forEach(idxAttEnunciado=> {
                                            ...
                                            attEnunciado.forEach(idxResp => { 
                                                ...
                                                {   
                                                    ...
                                                    { 
                                                        html += '<option value="'+idxValResp.id+'">'+idxValResp.valor+'</option>';
                                                    }else
                                                    {
                                                        if (idxValResp.valor != valRespAnterior){
                                                            html += '<option value="'+idxValResp.id+'">'+idxValResp.valor+'</option>';
                                                        }
                                                    }
                                                    });
...
function guardar(){ 
    ...
    {   
        ...
        let existenCambios = validarCambioDeRespuestas();
        
        if (existenCambios){
            Ext.Msg.confirm('Alerta','¿Está seguro de ejecutar el '+
            'derecho de '+strOpcion+' del titular '+ $(input).val()+
            ' con correo '+strListaCorreos+'?', function(btn){
            if(btn=='yes'){
                
                ejecutaPortabilidad(params);
            }
            });
        }
    }   
...
function ejecutaPortabilidad(params){
    var strListaCorreos = '';
            ...
            objDataClausula.forEach(idxClausula => {
                ...
                objDataAplicaDocumento.forEach(idxAplicaDoc => { 
                    ...
                    objDataRespuestaC.forEach(idxResp => { 
                        ...
                        var strcmbIdRespuesta = document.getElementById(strValorRespuestaCmb).value;
                       
                        if (idxResp.idRespuesta == strcmbIdRespuesta)
                        {
...
function validarCambioDeRespuestas()
{   
    var intCountExisteCambios = 0;
    let storeAdmiPoliticaClausula = Ext.StoreMgr.lookup('idStoreAdminPoliticas_Clausulas');
    var countDataGrid = storeAdmiPoliticaClausula.getCount();
    let storeRespuestaAnterior = Ext.StoreMgr.lookup('storeRespuestaAnterior');
    var countDataStoreRespAnt = storeRespuestaAnterior.getCount();

    if(countDataStoreRespAnt === 0)
    {
        //Cliente no tiene respuesta Anterior es su primera vez ingresando a derechos del titular
        return true;
    }
    else
    {
      ...
      {   
        ...
        var strValorRespAct = strcmbValorRespAct.options[strcmbValorRespAct.selectedIndex].innerText;

        if (strValorRespAct !== '' || strValorRespAct !== null)
        {
            intCountExisteCambios ++;
        }
               
      }
   
        if(intCountExisteCambios > 0)
        {
        ...
        {   
            Ext.Msg.alert('Error','Por favor seleccionar al menos una respuesta diferente');  
