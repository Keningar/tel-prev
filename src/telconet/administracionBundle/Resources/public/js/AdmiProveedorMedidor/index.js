...
Ext.onReady(function() { 
    ...
    Ext.define('ModelStore', {
        ...
        fields:
		[				
			{name:'idProveedorMedidor', mapping:'idProveedorMedidor'},
			{name:'nombreProveedorMedidor', mapping:'nombreProveedorMedidor'},
			{name:'estado', mapping:'estado'},
			{name:'action1', mapping:'action1'},
			{name:'action2', mapping:'action2'},
			{name:'action3', mapping:'action3'}                
		],
        idProperty: 'idProveedorMedidor'
    });
	...
	//****************  DOCKED ITEMS - BOTONES PARTE SUPERIOR ************************
        var permiso = '{{ is_granted("ROLE_123-8") }}';
        var boolPermiso1 = (typeof permiso === 'undefined') ? false : (permiso ? true : false);	
	
        var permiso = '{{ is_granted("ROLE_123-9") }}';
    ...
    grid = Ext.create('Ext.grid.Panel', {
        ...
        columns:[
                {
                  id: 'idProveedorMedidor',
                  header: 'idProveedorMedidor',
                  dataIndex: 'idProveedorMedidor',
                  hidden: true,
                  hideable: false
                },
                {
                  id: 'nombreProveedorMedidor',
                  header: 'Nombre Proveedor Medidor',
                  dataIndex: 'nombreProveedorMedidor',
                  width: 200,
                  sortable: true
                },
                {
                    ...
                    items: [
						{
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_123-6") }}';
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
								...
								if(rec.get('action1')!="icon-invisible")
									window.location = rec.get('idProveedorMedidor')+"/show";
								else
                        ...
                        {
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_123-4") }}';
                                    var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso ? true : false);
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
	                            ...
	                            if(rec.get('action2')!="icon-invisible")
	                                window.location = rec.get('idProveedorMedidor')+"/edit";
								else
									Ext.Msg.alert('Error ','No tiene permisos para realizar esta accion');
							}
                        },
                        {
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
								...
								var permiso = $("#ROLE_123-8");
								var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);							
								if(!boolPermiso){ rec.data.action3 = "icon-invisible"; }
								
								var permiso = $("#ROLE_123-9");
								var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);							
								if(!boolPermiso){ rec.data.action3 = "icon-invisible"; }
	                                ...
	                                Ext.Msg.confirm('Alerta','Se eliminara el registro. Desea continuar?', function(btn){
	                                    if(btn=='yes'){
	                                        Ext.Ajax.request({
	                                            url: "deleteAjax",
	                                            method: 'post',
	                                            params: { param : rec.get('idProveedorMedidor')},
            ...
            buttons: [
                ...
                items: [
                        { width: '10%',border:false},
                        {
                            xtype: 'textfield',
                            id: 'txtNombre',
                            fieldLabel: 'Nombre Proveedor Medidor',
                            value: '',
                            width: '200px'
                        },
