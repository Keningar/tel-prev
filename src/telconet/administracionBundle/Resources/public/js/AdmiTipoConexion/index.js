...
Ext.onReady(function() { 
    ...
    Ext.define('ModelStore', {
        ...
        fields:
		[				
			{name:'idTipoConexion', mapping:'idTipoConexion'},
			{name:'nombreTipoConexion', mapping:'nombreTipoConexion'},
			{name:'estado', mapping:'estado'},
			{name:'action1', mapping:'action1'},
			{name:'action2', mapping:'action2'},
			{name:'action3', mapping:'action3'}               
		],
        idProperty: 'idTipoConexion'
    });
	...
	//****************  DOCKED ITEMS - BOTONES PARTE SUPERIOR ************************
        var permiso = '{{ is_granted("ROLE_126-8") }}';
        var boolPermiso1 = (typeof permiso === 'undefined') ? false : (permiso ? true : false);	
	
        var permiso = '{{ is_granted("ROLE_126-9") }}';
    ...
    grid = Ext.create('Ext.grid.Panel', {
        ...
        columns:[
                {
                  id: 'idTipoConexion',
                  header: 'idTipoConexion',
                  dataIndex: 'idTipoConexion',
                  hidden: true,
                  hideable: false
                },
                {
                  id: 'nombreTipoConexion',
                  header: 'Nombre Tipo Conexion',
                  dataIndex: 'nombreTipoConexion',
                  width: 220,
                  sortable: true
                },
                {
                    ...
                    items: [
						{
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_126-6") }}';
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
								...
								if(rec.get('action1')!="icon-invisible")
									window.location = rec.get('idTipoConexion')+"/show";
								else
                        ...
                        {
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_126-4") }}';
                                    var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso ? true : false);
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
	                            ...
	                            if(rec.get('action2')!="icon-invisible")
	                                window.location = rec.get('idTipoConexion')+"/edit";
								else
									Ext.Msg.alert('Error ','No tiene permisos para realizar esta accion');
							}
                        },
	                                ...
	                                Ext.Msg.confirm('Alerta','Se eliminara el registro. Desea continuar?', function(btn){
	                                    if(btn=='yes'){
	                                        Ext.Ajax.request({
	                                            url: "deleteAjax",
	                                            method: 'post',
	                                            params: { param : rec.get('idTipoConexion')},
