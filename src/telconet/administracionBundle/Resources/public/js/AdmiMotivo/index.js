...
Ext.onReady(function() { 
    ...
    Ext.define('ModelStore', {
        ...
        fields:
		[				
			{name:'id_relacionsistema', mapping:'id_relacionsistema'},
			{name:'id_modulo', mapping:'id_modulo'},
			{name:'id_itemmenu', mapping:'id_itemmenu'},
			{name:'id_accion', mapping:'id_accion'},
			{name:'nombres_modulo', mapping:'nombres_modulo'},
			{name:'nombres_itemmenu', mapping:'nombres_itemmenu'},
			{name:'nombres_accion', mapping:'nombres_accion'},
			{name:'estado', mapping:'estado'},
			{name:'action1', mapping:'action1'},
			{name:'action2', mapping:'action2'},
			{name:'action3', mapping:'action3'}                  
		],
        idProperty: 'id_relacionsistema'
    });
    ...
    grid = Ext.create('Ext.grid.Panel', {
        ...
        columns:[
                {
                  id: 'id_relacionsistema',
                  header: 'IdRelacionSistema',
                  dataIndex: 'id_relacionsistema',
                  hidden: true,
                  hideable: false
                },
                {
                    ...
                    items: [
						{
	                        getClass: function(v, meta, rec) {
								var permiso = '{{ is_granted("ROLE_21-6") }}';
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
								...
								if(rec.get('action1')!="icon-invisible")
									window.location = rec.get('id_relacionsistema')+"/show";
								else
                        ...
                        {
	                        getClass: function(v, meta, rec) {
								var permiso = '{{ is_granted("ROLE_21-4") }}';
								var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso ? true : false);							
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
	                            ...
	                            if(rec.get('action2')!="icon-invisible")
	                                window.location = rec.get('id_relacionsistema')+"/edit";
								else
									Ext.Msg.alert('Error ','No tiene permisos para realizar esta accion');
							}
                        },
	                                ...
	                                Ext.Msg.confirm('Alerta','Se eliminara el registro. Desea continuar?', function(btn){
	                                    if(btn=='yes'){
	                                        Ext.Ajax.request({
	                                            url: "deleteAjax",
	                                            method: 'post',
	                                            params: { param : rec.get('id_relacionsistema')},
