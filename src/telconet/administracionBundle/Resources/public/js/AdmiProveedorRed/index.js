...
Ext.onReady(function() { 
	...
	//****************  DOCKED ITEMS - BOTONES PARTE SUPERIOR ************************
        var permiso = '{{ is_granted("ROLE_124-8") }}';
        var boolPermiso1 = (typeof permiso === 'undefined') ? false : (permiso ? true : false);	
	
        var permiso = '{{ is_granted("ROLE_124-9") }}';
    ...
    grid = Ext.create('Ext.grid.Panel', {
        ...
        columns:[
                {
                  id: 'idProveedorRed',
                  header: 'idProveedorRed',
                  dataIndex: 'idProveedorRed',
                  hidden: true,
                  hideable: false
                },
                {
                  id: 'nombreProveedorRed',
                  header: 'Nombre Proveedor Red',
                  dataIndex: 'nombreProveedorRed',
                  width: 200,
                  sortable: true
                },
                {
                    ...
                    items: [
						{
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_124-6") }}';
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
								...
								if(rec.get('action1')!="icon-invisible")
									window.location = rec.get('idProveedorRed')+"/show";
								else
                        ...
                        {
	                        getClass: function(v, meta, rec) {
                                    var permiso = '{{ is_granted("ROLE_124-4") }}';
                                    var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso ? true : false);
	                        ...
	                        handler: function(grid, rowIndex, colIndex) {
	                            ...
	                            if(rec.get('action2')!="icon-invisible")
	                                window.location = rec.get('idProveedorRed')+"/edit";
								else
									Ext.Msg.alert('Error ','No tiene permisos para realizar esta accion');
							}
                        },
	                                ...
	                                Ext.Msg.confirm('Alerta','Se eliminara el registro. Desea continuar?', function(btn){
	                                    if(btn=='yes'){
	                                        Ext.Ajax.request({
	                                            url: "deleteAjax",
	                                            method: 'post',
	                                            params: { param : rec.get('idProveedorRed')},
            ...
            buttons: [
                ...
                items: [
                        { width: '10%',border:false},
                        {
                            xtype: 'textfield',
                            id: 'txtNombre',
                            fieldLabel: 'Nombre Proveedor Red',
                            value: '',
                            width: '200px'
                        },
