...
Ext.onReady(function(){
    //deshabilito el tipo porque si se edita causa problemas de inconsistencia
    document.getElementById('telconet_schemabundle_admimodeloelementotype_tipoElementoId').disabled= true;
...
function validarFormulario()
{
    ...
    //habilito
    document.getElementById('telconet_schemabundle_admimodeloelementotype_tipoElementoId').disabled= false;
    
    var strTipoElemento = $("#telconet_schemabundle_admimodeloelementotype_tipoElementoId option:selected").text();

    if( strTipoElemento != 'VEHICULO' && strTipoElemento != 'TABLET' && strTipoElemento != 'FUSIONADORAS' )
    {
...
$("#opcionesCamara").hide();
window.onload = selecionarTipoElemento;

function selecionarTipoElemento() 
{
  //id tomado desde la etiqueta HTML
  const comboTipoElemento = document.getElementById(
    "telconet_schemabundle_admimodeloelementotype_tipoElementoId"
  );
  const tipoElemento =
    comboTipoElemento.options[comboTipoElemento.selectedIndex].text;
  $.ajax({
    type: "GET",
    url: "getCamarasValidasSafecity",
    async: false,
    success: function (camarasValidas) {
      arrayCamarasValidas = JSON.parse(camarasValidas);
    },
  });
  //Verificamos que el tipo de elemento selecdionado sea una camara safecity valida
  var boolCamaraValida = false;
  arrayCamarasValidas.map((camaras) => {
    if (tipoElemento == camaras.valor1) 
    {
  ...
  if (boolCamaraValida) 
  {
    ...
    $.ajax({
      type: "GET",
      url: "getTiposCamara",
      success: function (tipoCamara) {
        arrayTipoCamara = JSON.parse(tipoCamara);
        const comboTipoCamara = document.getElementById("tipoCamara");

        arrayTipoCamara.map((tipo) => {
          const opcionTipoCamara = document.createElement("option");
          opcionTipoCamara.id = tipo.valor1;
          opcionTipoCamara.value = tipo.valor1;
          opcionTipoCamara.text = tipo.valor1;
          comboTipoCamara.appendChild(opcionTipoCamara);
        });
        //Seteamos por defecto la configuracion actual de resolucion y tipo de camara que posee el modelo
        establecerValoresCamaras();
      },
    });
    //Combo Resolucion Camara
    $.ajax({
      type: "GET",
      url: "getResolucionCamara",
      success: function (resolucionCamara) {
        const arrayResolucionCamara = JSON.parse(resolucionCamara);
        const comboResolucionCamara =
          document.getElementById("resolucionCamara");

        arrayResolucionCamara.map((resolucion) => {
          const opcionResolucionCamara = document.createElement("option");
          opcionResolucionCamara.id = resolucion.id;
          //Verificamos que los valores alternos de la resolucion existan, si no se muestra la resolucion clasica
          const res =
            resolucion.valor2 && resolucion.valor3
              ? resolucion.valor2 + "x" + resolucion.valor3
              : resolucion.valor1;
          opcionResolucionCamara.value = resolucion.valor1;
          opcionResolucionCamara.text = res;
          comboResolucionCamara.appendChild(opcionResolucionCamara);
        });
        //Seteamos por defecto la configuracion actual de resolucion y tipo de camara que posee el modelo
        establecerValoresCamaras();
      },
 ...
 {
    //Limpiamos los combos cuando el elemento no es una camara
    const comboResolucionCamara = document.getElementById("resolucionCamara");
    for (let i = comboResolucionCamara.options.length; i >= 0; i--) 
    {
      comboResolucionCamara.remove(i);
    }

    const comboTipoCamara = document.getElementById("tipoCamara");
    for (let i = comboTipoCamara.options.length; i >= 0; i--) 
    {
      comboTipoCamara.remove(i);
    }
    $("#opcionesCamara").hide();
  } //if(boolCamaraValida)
}
function establecerValoresCamaras(){
    $.ajax({
        type: "GET",
        data: {
          modeloElemento: document.getElementById(
            "telconet_schemabundle_admimodeloelementotype_nombreModeloElemento"
          ).value,
        },
        url: "getParametrosDetCamaras",
        success: function (parametros) {
          //Selecionamos por defecto el valor corespondiente
          
          const comboTipoCamara = document.getElementById("tipoCamara");
          const comboResolucionCamara = document.getElementById("resolucionCamara");
          const arrayParametro = JSON.parse(parametros);
  
          if (arrayParametro.length !==0) 
          {
            comboTipoCamara.value = arrayParametro["tipo"];
            comboResolucionCamara.value = arrayParametro["resolucion"];
          }
          else
          {
            comboTipoCamara.appendChild(document.createElement("option")).text =
              "Seleccione";
            comboResolucionCamara.appendChild(
              document.createElement("option")
            ).text = "Seleccione";
            comboResolucionCamara.value = "Seleccione";
            comboTipoCamara.value = "Seleccione";
          }
        },
      });
}