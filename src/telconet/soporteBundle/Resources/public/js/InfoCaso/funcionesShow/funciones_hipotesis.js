function agregarHipotesis(id_caso,numero,fecha,hora,version_inicial){
	    ...
	    btnguardar = Ext.create('Ext.Button', {
			...
			handler: function() {
				...
				if(valorBool)
				{
					...
					if(json_hipotesis == "")Ext.Msg.alert('Alerta','No puede agregar hipótesis Iguales');
					      ...
					      conn.request({
						      ...
						      url: '../actualizarHipotesis',
						      success: function(response){
                                  cierraVentanaByIden(winHipotesis);
							      Ext.Msg.alert('Mensaje','Se actualizaron las hipótesis.', function(btn){
								      if(btn=='ok'){									      										
	    ...
	    storeSintomas = new Ext.data.Store({ 
	        ...
	        listeners: {
				...
				comboHipotesisStore = new Ext.data.Store({ 
	                             ...
	                             fields:
                                    ...
                                    listeners: {
                                        ...
                                        load: function(store) {
                                            if(store.getCount() == 0)
                                            {
                                                Ext.Msg.alert('Alerta ', 'No hay hipótesis asociadas a este tipo de caso');
                                            };
                                        }
                                    }
				});
							...
							//Si la bandara de buscar por arbol de hipotesis es S convierte el comboBoxField en un displayField
                            if (strBuscaPorArbolHipotesis === 'S')
                            {
	                            combo_hipotesis = new Ext.form.DisplayField( {
                                    xtype: 'displayfield',
                                    name : 'home_score',
                                    value: ''
                                });
...
// 												{
		                                    ...
		                                    handler: function(grid, rowIndex, colIndex) {
												...
												asignarCaso(rec, id_caso, numero, fecha, hora, version_inicial);
		                                    }
										},
                                        {
                                            getClass: function(v, meta, rec)
                                            {
                                                var cssName           = "icon-invisible";
                                                this.items[0].tooltip = 'Agregar Hipótesis';
                                                if (strBuscaPorArbolHipotesis === 'S')
                                                {
                                                    cssName = "button-grid-agregarHipotesis";
                                                }
                                                if (cssName == "icon-invisible")
                                                {
                                                    this.items[0].tooltip = '';
                                            ...
                                            tooltip: 'Agregar Hipótesis',
                                            handler: function(grid, rowIndex, colIndex){
                                                var rec =  gridHipotesis.getStore().getAt(rowIndex);
                                                agregarArbolHipotesis(rec, id_caso);
                                            }
                                        }
									]
		                         }
	                         ...
	                         dockedItems: [{
	                                 ...
	                                 items: [{
	                                         ...
	                                         disabled: false,
	                                         handler : function(){									
												if((flagCreador && !flagBoolAsignado) || (flagBoolAsignado && flagAsignado) || (mostrarHipotesis == "S"))
												{								
													if((flagTareasAbiertas && flagTareasSolucionadas) )
													{
														...
														{
															Ext.Msg.alert('Alerta ', "Debe completar datos de las hipótesis a ingresar, antes de solicitar una nueva hipótesis");
														}
														else if(!boolSigue2)
														{
															Ext.Msg.alert('Alerta ', "No puede ingresar el mismo sintoma e hipótesis! Debe modificar el registro repetido, antes de solicitar una nueva hipótesis");
														}
														else if(esOrigen > 0)
														{
															Ext.Msg.alert('Alerta ', "No puede crear más de una hipótesis");
														}
														else
														{
															Ext.Msg.alert('Alerta ', "Debe completar datos de las hipótesis a ingresar, antes de solicitar una nueva hipótesis");
													...
													{
														Ext.Msg.alert('Alerta ', "No puede crear Hipótesis, porque tiene tareas abiertas.");
													}
												}
												else
												{
													Ext.Msg.alert('Alerta ', "No tiene permisos para crear Hipótesis, porque el caso fue asignado a otra persona");
												}																									
	                                         }
	                                     }]
	                                 }],
	                             ...
	                             title: 'Ingresar Información de Hipótesis'
							});
								...
								[
	                                ...
	                                items: [
                                        ...
                                        },
                                        {
                                            getClass: function(v, meta, rec)
                                            {
                                                var cssName = "button-grid-agregarHipotesis";
                                                if (cssName == "icon-invisible")
                                                    this.items[0].tooltip = '';
                                                else 
                                                    this.items[0].tooltip = 'Agregar Hipótesis';
...
function agregarAfectadosXHipotesis(hipotesis, id_caso){
    ...
    if(hipotesis=='')
    {
        Ext.Msg.alert('Alerta','Debe escoger una hipótesis primero.');
        return;
    }   
...
function validarHipotesisNoRepetidas(nombre , array){
  
      cont =0;
      for(var i=0; i < array.getCount(); i++){
	
	if(nombre == array.getAt(i).data['nombre_hipotesis'])cont++;
...
function obtenerHipotesis()
{
  ...
  for(var i=0; i < gridHipotesis.getStore().getCount(); i++)
  {
  	if(!validarHipotesisNoRepetidas(gridHipotesis.getStore().getAt(0).data['nombre_hipotesis'],gridHipotesis.getStore()))return "";	  			
...
function agregarArbolHipotesis(rec, id_caso){
	...
	storeArbolHipotesisNivel1 = new Ext.data.Store(objStoreArbolHipotesis);
	storeArbolHipotesisNivel1.proxy.extraParams = {padreHipotesis: '0',estado:'Activo'};
	storeArbolHipotesisNivel2 = new Ext.data.Store(objStoreArbolHipotesis);
	storeArbolHipotesisNivel3 = new Ext.data.Store(objStoreArbolHipotesis);
	...
	formPanelArbolHipotesis = Ext.create('Ext.form.Panel', {
		bodyPadding: 5,
		waitMsgTarget: true,
		fieldDefaults: {
			labelAlign: 'left',
		...
		buttons: [{
			...
			formBind: true,
			handler: function(){
				rec.data.nombre_hipotesis = Ext.getCmp('comboArbolHipotesisNivel3').getRawValue();
				rec.data.id_hipotesis  = Ext.getCmp('comboArbolHipotesisNivel3').value;
				rec.commit();
				gridHipotesis.getView().refresh();
				winAgregarArbolHipotesis.destroy();
			}
		},{
			text: 'Cancelar',
			handler: function(){
	...
	winAgregarArbolHipotesis = Ext.create('Ext.window.Window', {
		title: 'Agregar Hipótesis',
		modal: true,
		closable: false,
		width: 650,
		layout: 'fit',
		items: [formPanelArbolHipotesis]
	}).show();
}