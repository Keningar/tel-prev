...
function getElementoAfectadoXDetalle(id_caso){
  
	var elementoAfectado = '';
	...
	conn.request({
            ...
            url: 'getElementoAfectadoXDetalle',
            success: function(response)
            {			
                elementoAfectado = response.responseText;			
                return elementoAfectado;
            },
...
{  
	...
	var esDepartamento = rec.get('esDepartamento');		
	var flagBoolAsignado = rec.get('flagBoolAsignado');
	var flagAsignado = rec.get('flagAsignado');	
    var fecha = rec.get('date');
    var empresa = rec.get('empresa');

    //Se obtiene la fecha y hora del servidor    
    var arrayFecha = fecha.split(" ");
	
	var flagAregarTarea = 'S';
    
    var arrayUltimaMilla = new Array();		
	...
	conn.request({
        ...
        {			
            ...
            arrayUltimaMilla = elementoAfectado.split(":");                        
                        ...
                        load: function(sender, node, records) {
                            ...
                            Ext.each(records, function(record, index) {
                                ...
                                if (storeHipotesis.getCount() > 0) {
                                    ...
                                    gridHipotesis2 = Ext.create('Ext.grid.Panel', {
                                        id: 'gridHipotesis',
                                        ...
                                        height: 150,
                                        frame: true,
                                        title: 'Informacion de Hipotesis 2'
                                    });
                                    ...
                                    Ext.getCmp('searchTipo_cmp').setValue(5);
                                    Ext.getCmp('searchTipo_cmp').setRawValue('Prueba');
                                                            ...
                                                            }else
                                                            {
                                                                conn.request({
                                                                    ...
                                                                    success: function(response)
                                                                    {
                                                                        ...
                                                                        if (json.success)
                                                                        {
                                                                            /*agregarAsignacionXTarea();
                                                                            winAgregarAsignacionTarea.destroy();*/
                                                                            agregarAsignacionXTarea(id_caso,
...
function agregarAsignacionXTarea(id_caso, tarea, fechaActual , horaActual)
{
                     ...
                     {                    
                        ...
                        {		
                           ...
                           // tarea.data.id_refAsignado = Ext.getCmp('comboEmpleado').value;
                            ...
                            var valPrefijo = Ext.getCmp('prefijoNumeroClienteARecibir').getValue() != null ? Ext.getCmp('prefijoNumeroClienteARecibir').getValue() : "";
                            var valNombreClienteARecibir = Ext.getCmp('nombreClienteARecibir').getValue();
                            var valTelefonoClienteARecibir = Ext.getCmp('telefonoClienteARecibir').getValue();
                            var valCargoClienteARecibir = Ext.getCmp('cargoClienteARecibir').getValue();
                            var valCorreoClienteARecibir = Ext.getCmp('correoClienteARecibir').getValue();
    ...
    var tabs = new Ext.TabPanel({
        ...
        items: [formPanel,panelHal(id_caso, tarea, fechaActual)]
    });
...
function administrarTareas(rec){ 
	...
	var hora = rec.get('hora_apertura');
	var version_inicial = rec.get('version_ini');
    var empresa_caso    = rec.get('empresa');
                                    ...
                                    conn.request({
                                        ...
                                        success: function(response)
                                        {
                                            ...
                                            if (json.success)
                                            {
                                                ...
                                                if (rs !== -1)
                                                {
                                                    finalizarTarea(id_caso, numero, grid.getStore().getAt(rowIndex).data,fechaActual,json.horaActual,
                                                        rec.data.tipoAsignado,rec.data.id_cuadrilla);
                                                }
                                            }
                                            else
                                            {
                        ...
                        },
				{
					getClass: function(v, meta, rec) {
						...
						if (rec.get('action5') == "icon-invisible") 
							...
							this.items[5].tooltip = 'Cancelar Tarea';
...
function agregarSeguimiento(id_caso,numero,data){    
    ...
    btnguardar2 = Ext.create('Ext.Button', {
            ...
            handler: function() {
               ...
               // json_tareas = obtenerTareas();
               var obSeguimiento = Ext.getCmp('seguimiento').value;
...
//function obtenerDatosCasosCierre(id_caso,conn,esCancelado){
//       
//      conn.request({
//	    method: 'POST',
//	    params :{
...
//		    if(json)
//		    {			    	  
//				  cerrarCaso(null,json.encontrados[0]);										    
//		    }
//		    else
//		    {
//			    Ext.Msg.alert('Alerta ',json.mensaje);
...
function finalizarTarea(id_caso,numero,data,fechaActual,horaActual,tipoAsignado,idCuadrilla)
{
	...
	esSolucion = Ext.getCmp('radio1').getValue();
            ...
            success: function(response)
            {
                    ...
                    if (json.success)
                    {
                        Ext.Msg.alert('Mensaje', 'Se finalizó la tarea.', function(btn)
                        {
                            ...
                            {
                                winAdministrarTareas.destroy();
                                //Se determina analisis de cierre de caso siempre y cuando no existan tareas abiertas y ademas
                                //haya al menos una finalizada que haya dado solucion al caso
...
function subirMultipleAdjuntosCaso(rec)
{
    var id_caso = rec.get('id_caso');
    ...
    var panelMultiupload = Ext.create('widget.multiupload');
    var formPanel = Ext.create('Ext.form.Panel',
     {
        width: 500,
        frame: true,
...
function panelHal(idCaso, tarea, fechaActual) {

    var idAdmiTarea = "'"+tarea.data.id_tarea+"'";

    var radbuttonHal = '<div align="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n\
                       &nbsp;<input type="radio" onchange="opcionesHal(1,'+idAdmiTarea+','+idCaso+','+tarea.data.id_hipotesisTarea+');\n\
                       " value="halDice" name="radioCuadrilla" id="radio_a">&nbsp;\n\
                       Mejor Opci&oacuten&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" \n\
                       onchange="opcionesHal(2,'+idAdmiTarea+','+idCaso+','+tarea.data.id_hipotesisTarea+');" value="halSugiere" \n\
            ...
            items : [
                ...
                { xtype: 'tbfill' },
                {
                    ...
                    handler  : function()
                    {
                        ...
                        storeIntervalosHal.getProxy().extraParams.idCaso        = idCaso;
        ...
        [
            {
                ...
                handler: function()
                {
                    ...
                    else
                    {
                       ...
                       {
                           tarea.data.idSugerencia   = selModelHalSugiere.getSelection()[i].data.idSugerencia;
                           tarea.data.fechaEjecucion = selModelHalSugiere.getSelection()[i].data.fecha;
                           tarea.data.horaEjecucion  = selModelHalSugiere.getSelection()[i].data.horaIni;
                           tarea.data.fechaVigencia  = selModelHalSugiere.getSelection()[i].data.fechaVigencia;
                       }
                    }
                    ...
                    var valPrefijo = Ext.getCmp('prefijoNumeroClienteARecibirHal').getValue() != null ? Ext.getCmp('prefijoNumeroClienteARecibirHal').getValue() : "";
                    var valNombreClienteARecibir = Ext.getCmp('nombreClienteARecibirHal').getValue();
                    var valTelefonoClienteARecibir = Ext.getCmp('telefonoClienteARecibirHal').getValue();
                    var valCargoClienteARecibir = Ext.getCmp('cargoClienteARecibirHal').getValue();
                    var valCorreoClienteARecibir = Ext.getCmp('correoClienteARecibirHal').getValue();
                    var valConvencionalClienteARecibir = Ext.getCmp('convencionalClienteARecibirHal').getValue() != ''
                        ? valPrefijo + Ext.getCmp('convencionalClienteARecibirHal').getValue()
                        : '';
...
function opcionesHal(tipo,tarea,caso,hipotesis)
{
    ...
    storeIntervalosHal.getProxy().extraParams.tipoHal       = tipo;
    storeIntervalosHal.load();
    Ext.getCmp('fecha_sugerida').setValue(null);
    Ext.getCmp('hora_sugerida').setValue(null);
}
