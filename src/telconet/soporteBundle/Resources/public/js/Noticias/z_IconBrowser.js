...
Ext.define('Ext.chooser.IconBrowser', {
        ...
        // '<div class="details">',
            '<tpl for=".">',
                '<div class="thumb-wrap">',
                    '<div class="thumb">',
                    (!Ext.isIE6? '<img src="{imagen_mini}" width="130" height="130" />' : 
                    '<div style="width:74px;height:74px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'{imagen_mini}\')"></div>'),
                    '</div>',
                    '<span>{name}</span>',
                '</div>',
            '</tpl>'
    ...
    initComponent: function() {
        this.store = Ext.create('Ext.data.Store', {
            autoLoad: true,
            fields: ['name', 'thumb', 'url', 'type', 'ext', 'extension', 'dimension', 'peso','host_base','imagen_mini','imagen_media'],
            proxy: {
                type: 'ajax',
