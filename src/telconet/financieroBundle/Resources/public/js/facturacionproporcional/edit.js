...
Ext.define('ListadoDetalleOrden', {
 ...
 store = Ext.create('Ext.data.Store', {
    ...
    proxy: {
        ...
        url: url_info_detalle_factura,
        reader: {
            type: 'json',
            root: 'listadoInformacion'
            // records will have a 'plant' tag
        },
        extraParams:{facturaid:factura_id,puntoid:punto_id,precargado:''},
        simpleSortMode: true               
    },
});
...
function verificarCheck(info)
{
    if(info=='Orden')
    {
		...
		store.getProxy().extraParams.precargado= "P";
        store.load();
...
function agregar_detalle_portafolio()
{
    ...
    var rec = new ListadoDetalleOrden({'codigo':producto[0],'informacion':producto[1],'precio':precio_total,'cantidad':cantidad,'descuento':descuento,'tipo':"PL"});
    store.add(rec);
