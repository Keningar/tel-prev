...
Ext.onReady(function(){
            ...
            DTFechaTransferencia = new Ext.form.DateField({
                    id: 'fechaTransferencia',
                    fieldLabel: '',
                    labelAlign : 'left',
                    xtype: 'datefield',
                    format: 'Y-m-d',
					renderTo: 'fecha_transferencia',					
                    width:325
            });			
    ...
    combo_bancos_transferencia = new Ext.form.ComboBox({
            ...
            queryMode:'local',
            width: 200,
            emptyText: 'Seleccione Banco Transferencia',
            store:storeBancosContables,
            displayField: 'descripcion_banco',
            valueField: 'id_banco',
   ...
   Ext.define('valoresFacturaModel', {
    ...
    storeValoresFact = Ext.create('Ext.data.Store', {
            ...
            listeners: {
					...
					var newnumber = new Number((record.data.valorFactura-record.data.totalPagos)+'').toFixed(parseInt(2));
...
var pagos=0;var pagosFacturaGrid=0;
...
if($('#infopagodettype_factura').val() && $('#infopagodettype_valor').val()){

pagosFacturaGrid=new Number(parseFloat( obtienePagosPorFacturaEnGrid($('#infopagodettype_factura').val())  ));
//console.log("PagosFactura:"+pagosFacturaGrid);

pagos=new Number(parseFloat(storeValoresFact.getAt(0).data.totalPagos) + parseFloat(valorPago)).toFixed(parseInt(2));
var valor=0;

if(storeValoresFact.getAt(0).data.valorFactura)
    valor=new Number(parseFloat(storeValoresFact.getAt(0).data.valorFactura) - pagos - pagosFacturaGrid).toFixed(parseInt(2));    
				   ...
				   if (textFormaDePago.toUpperCase()=='DEPOSITO'){           
					 ...
					 feProceso=Ext.Date.format(Ext.getCmp('fechaDeposito').getValue(),'Y-m-d');
					   ...
					   if (textFormaDePago.toUpperCase()=='TRANSFERENCIA'){           
						 ...
						 feProceso=Ext.Date.format(Ext.getCmp('fechaTransferencia').getValue(),'Y-m-d');
					   }				   
				   }
...
//##Valida campos segun forma de pago
    ...
    if(
        ...
        && banco && tipoCuenta && numeroCuenta && $('#infopagodettype_valor').val() && feProceso)
    {
        validacionOk=true;
        rec = new InfoPagoDetModel({'formaPago':$('#infopagodettype_forma_pago').val(),
            ...
            'numeroCta':numeroCuenta,'valor':$('#infopagodettype_valor').val(),'comentario':$('#infopagodettype_comentario').val(),'feProceso':feProceso});
    ...
    else
    {
        ...
        Ext.Msg.alert('Alerta ','El pago excede el saldo de la factura. No puede agregarse detalle de pago.');            
    }    
}else
{
