...
Ext.onReady(function(){
     ...
     * Opcion que permite presentar detalle de campos del detalle del pago
    ...
    {   
        ...
        if (!winEditarDetalle)
        {
            ...
            if(rec.data.formaPago == 'DEBITO BANCARIO')
                labelNumeroReferencia = "Numero Debito";
            ...
            formPanelEditarDetalle = Ext.create('Ext.form.Panel', 
            {
                ...
                [{
                    ...
                    handler: function()
                    {
                        ...
                        parametros = 
                            { 
                                comentario: Ext.getCmp('new_comentario').value,
                                formaPago: rec.data.formaPago, idPago: rec.data.idPago, 
                                idPagoDet: rec.data.idPagoDet,idBanco: Ext.getCmp('cmbBancos').value , 
                        ...
                        parametros = { comentario: Ext.getCmp('new_comentario').value,
                            formaPago: rec.data.formaPago,idPago: rec.data.idPago, 
                            idPagoDet: rec.data.idPagoDet, idBanco: Ext.getCmp('cmbBancos').value , 
                            tipoCuenta: Ext.getCmp('cmbTipoCuenta').value ,
                            numeroReferencia: Ext.getCmp('new_numero_referencia').value , 
                        ...
                        if(rec.data.formaPago == 'RECAUDACION' || rec.data.formaPago == 'DEBITO BANCARIO')
                        {
                            ...
                            { 
                                formaPago: rec.data.formaPago,idPago: rec.data.idPago, 
                                idPagoDet: rec.data.idPagoDet, fechaDeposito: Ext.getCmp('fechaDeposito').value,
                                numeroReferencia: Ext.getCmp('new_numero_referencia').value
                            };
                        }
                        if(rec.data.formaPago == 'RETENCION FUENTE 1%' 
                        ...
                        if(rec.data.formaPago == 'TARJETA DE CREDITO')
                        {
                            ...
                            { 
                                comentario: Ext.getCmp('new_comentario').value,
                                formaPago: rec.data.formaPago,idPago: rec.data.idPago, 
                                idPagoDet: rec.data.idPagoDet,idBanco: Ext.getCmp('cmbBancosTarjeta').value , 
                                tipoCuenta: Ext.getCmp('cmbTipoCuentaTarjeta').value ,
                                numeroReferencia: Ext.getCmp('new_numero_referencia').value 
                            };
                        }

                        if(rec.data.formaPago == 'EFECTIVO')
                        {
                            parametros = 
                            { 
                                comentario: Ext.getCmp('new_comentario').value,
                ...
                {
                    text: 'Cerrar',
                    handler: function(){winEditarDetalle.close();}
                }]
            });
