...
Ext.onReady(function () {
    ...
    /*Inicio Facturas Proporcionales*/
    storeResumenFacturasProporcionales = Ext.create('Ext.data.JsonStore', {
        ...
        proxy: {
            ...
            extraParams: {dateFechaDesde: '', dateFechaHasta: '', intIdOficina: '', strTipoDocumento: 'FACP'},
            simpleSortMode: true
        },
        listeners: {
            beforeload: function (store) {
                storeResumenFacturasProporcionales.getProxy().extraParams.dateFechaDesde = Ext.getCmp('dateFechaDesde').getValue();
                storeResumenFacturasProporcionales.getProxy().extraParams.dateFechaHasta = Ext.getCmp('dateFechaHasta').getValue();
                storeResumenFacturasProporcionales.getProxy().extraParams.intIdOficina   = Ext.getCmp('comboOficinas').getValue();
            },
            load: function (store) {
            }
        }
    ...
    var panel = Ext.create('widget.panel', {
        width: 800,
        height: 400,
        title: 'Resumen Facturación Proporcional Electronica',
        renderTo: 'resumenFACPElectronica',
        layout: 'fit',
        items: {
            xtype: 'chart',
            animate: true,
            shadow: true,
            store: storeResumenFacturasProporcionales,
            autoScroll: false,
            legend: {
                position: 'top'
            },
            axes: [{
                    ...
                    title: 'Total de Facturas Proporcionales',
                    grid: true,
                    label: {
                        renderer: function (v) {
    ...
    Ext.create('Ext.grid.Panel', {
        renderTo: 'resumenDetFACPElectronica',
        store: storeResumenFacturasProporcionales,
        width: 642,
        height: 200,
        title: 'Resumen Facturación Proporcional Electronica',
...
function Buscar() {
    if ((Ext.getCmp('dateFechaDesde').getValue()) && (Ext.getCmp('dateFechaHasta').getValue()))
    {
        ...
        {
            storeResumenFacturasProporcionales.getProxy().extraParams.dateFechaDesde = Ext.getCmp('dateFechaDesde').value;
            storeResumenFacturasProporcionales.getProxy().extraParams.dateFechaHasta = Ext.getCmp('dateFechaHasta').value;
            storeResumenFacturasProporcionales.getProxy().extraParams.intIdOficina   = Ext.getCmp('comboOficinas').value;
            storeResumenFacturasProporcionales.load();
        }
    }
    else
    {
