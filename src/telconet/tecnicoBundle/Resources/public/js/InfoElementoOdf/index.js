Ext.onReady(function() {
    ...
    store = new Ext.data.Store({
        ...
        fields:
            [
                ...
                {name: 'strOdfFactAutomatica', mapping: 'strOdfFactAutomatica'},                
        ...
        columns: [
            ...
            },
            {
                ...
                tpl: '<span class="box-detalle">{nombreElemento}</span>\n\\n\
                        <span class="bold">Factibilidad Automatica:</span><span>{strOdfFactAutomatica}</span></br>\n\
                        <span class="bold">Jurisdiccion:</span><span>{jurisdiccionNombre}</span></br>\n\
                        <span class="bold">Canton:</span><span>{cantonNombre}</span></br>\n\\n\
                        <span class="bold">UdRack:</span><span>{nombreElementoNodo}</span></br>\n\
                ...
                items: [
                    //VER ODF
                    {
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            if (rec.get('action1') != "icon-invisible")
                                window.location = "" + rec.get('idElemento') + "/showOdf";
                            else
                    ...
                    //EDITAR ODF
                    {
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            var rec = store.getAt(rowIndex);
                            if (rec.get('action2') != "button-grid-invisible")
                                window.location = "" + rec.get('idElemento') + "/editOdf";
                        }
                    },
                    //ELIMINAR ODF
                    {
                        getClass: function(v, meta, rec) {
                            var permiso = $("#ROLE_275-8");
                            var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);
                            if (!boolPermiso) {
                                return 'button-grid-invisible';
                            }
    ...
    function administrarPuertos(data) {
        ...
        var storeInterfaces = new Ext.data.Store({
            ...
            proxy: {
                type: 'ajax',
                url: url_interfaceOdf,
                extraParams: {idElemento: data.idElemento, tipo: 'ODF'},
                reader: {
                    type: 'json',
                    totalProperty: 'total',
                    root: 'encontrados'
