...
//    alert(Variables[n-1]);
    var idCaja = Variables[n-2];
    
//    alert(idDslam);
    
    var accion="cargarDatosCaja";
          ...
          success: function(response)
                   {
                     ...
                     if(json.total>0){
                         ...
                         agregarValue("telconet_schemabundle_infoelementocajatype_jurisdiccionId", json.encontrados[0]['idJurisdiccion']);
                         agregarValue("telconet_schemabundle_infoelementocajatype_nivel",json.encontrados[0]['nivel']);
                         agregarValue("telconet_schemabundle_infoelementocajatype_ubicadoEn",json.encontrados[0]['strUbicado']);
                         agregarValue("telconet_schemabundle_infoelementocajatype_idEdificacion",json.encontrados[0]['id']);
                         presentarCantonesEdit(Ext.get('telconet_schemabundle_infoelementocajatype_jurisdiccionId'), "telconet_schemabundle_infoelementocajatype_cantonId", "buscarCantones", "encontrados", "",json.encontrados[0]['idCanton'], json.encontrados[0]['idParroquia']);
                     }
                     else{
                         alert("sin datos");
                     }
          ...
          success: function(response)
                   {
                     ...
                     if(root == 'encontrados')
                     {
                       llenarCombo(objeto_2, json.encontrados, 'encontrados', seleccion, 'nombre_canton', 'id_canton', 'telconet_schemabundle_infoelementocajatype_cantonId', valor_campo);
                       
                       presentarParroquiasEdit(Ext.get('telconet_schemabundle_infoelementocajatype_cantonId'), "telconet_schemabundle_infoelementocajatype_parroquiaId", "buscarParroquias", "encontrados", "",valor_campo1);
                     }
                   },
          failure: function(result)
                   {
          ...
          success: function(response)
                   {
                     ...
                     if(root == 'encontrados')
                     {
                       llenarCombo(objeto_2, json.encontrados, 'encontrados', seleccion, 'nombre_parroquia', 'id_parroquia', 'telconet_schemabundle_infoelementocajatype_parroquiaId', valor_campo);
