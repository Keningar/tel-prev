/**
 * Funcion que sirve para cargar la data en el grid de elementos-switch
 * 
...
Ext.onReady(function() {
    ...
    var storeMarcas = new Ext.data.Store({ 
        ...
        proxy: {
            type: 'ajax',
            url : getMarcasElementosSwitches,
            extraParams: {
                tipoElemento: 'SWITCH'
            },
            reader: {
                    ...
                    items: [
                        //SHOW ELEMENTO
                        {
                            ...
                            handler: function(grid, rowIndex, colIndex) {
                                ...
                                window.location = ""+rec.get('idElemento')+"/showSwitch";
                            }
                        },
                        //EDITAR ELEMENTO
                        {
                            getClass: function(v, meta, rec) {
                                var permiso = $("#ROLE_315-5");
                                var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);          
                            ...
                            handler: function(grid, rowIndex, colIndex) {
                                var rec = store.getAt(rowIndex);
                                if(rec.get('action2')!="button-grid-invisible"){
                                    window.location = ""+rec.get('idElemento')+"/editSwitch";
                                }
                            }
                        },
                        //ELIMINAR ELEMENTO
                ...
                Ext.Ajax.request({
                    ...
                    success: function(response){
                        ...
                        if(text=="OK"){
                            ...
                            Ext.Msg.alert('Error ','Error: Aun No existe la Herramienta!' );
                        }
                    },
                    failure: function(result)
                    {
