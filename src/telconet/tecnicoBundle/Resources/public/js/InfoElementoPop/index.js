...
//        autoLoad:true,
        proxy: {
            type: 'ajax',
            url : '../../../administracion/tecnico/admi_marca_elemento/getMarcasElementosPop',
            reader: {
                type: 'json',
                totalProperty: 'total',
                root: 'encontrados'
                ...
                },
                {
                    ...
                    items: [{
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            window.location = ""+rec.get('idElemento')+"/showPop";
                            }
                        },
                        {
                        getClass: function(v, meta, rec) {
                            var permiso = $("#ROLE_150-5");
                                var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);          
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            var rec = store.getAt(rowIndex);
                            if(rec.get('action2')!="button-grid-invisible")
                                window.location = ""+rec.get('idElemento')+"/editPop";
                                //alert(rec.get('nombre'));
                            }
                        },
                        {
                        getClass: function(v, meta, rec) {
                             var permiso = $("#ROLE_150-8");
                                var boolPermiso = (typeof permiso === 'undefined') ? false : (permiso.val() == 1 ? true : false);          
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            if(rec.get('action3')!="button-grid-invisible")
                                Ext.Msg.confirm('Alerta','Se eliminara el registro. Desea continuar?', function(btn){
                                    if(btn=='yes'){
                                        Ext.Ajax.request({
                                            url: ""+rec.get('idElemento')+"/deletePop",
                                            method: 'post',
