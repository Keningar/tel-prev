...
Ext.onReady(function() { 
    ...
    combo_regiones.value=valueRegion;
    combo_regiones.setRawValue(valueRegion);
    ...
    combo_filiales.value=idFilial;
    combo_filiales.setRawValue(nombreFilial);

    //Es monitorizado    
    if(!Ext.isEmpty(esMonitoreado))
    {
        esMonitoreado == 'S'?document.getElementById("esM").checked = true:document.getElementById("esM").checked = false;
    }
    else
    {
        document.getElementById("esM").checked = false;
    }
    ...
    var fechaIniEditContrato=document.getElementById("fecha_inicio_contrato").value.trim();
    if(fechaIniEditContrato=="")
    {
        fechaIniEditContrato=new Date();
    }
    ...
    fechaInicioContrato =   Ext.create('Ext.form.Panel', {
                                ...
                                items: [{
                                    ...
                                    anchor: '100%',
                                    format: 'Y-m-d',
                                    value: fechaIniEditContrato,
                                    maxValue: new Date()  // limited to the current date or prior
        }]
    });
    
    var fechaFinEditContrato=document.getElementById("fecha_fin_contrato").value.trim();
    if(fechaFinEditContrato=="")
    {
        fechaFinEditContrato=new Date();
    }
    ...
    {
        tiposHtml = '<input type="radio" onchange="cambiarTipoDeVehiculo(this.value);" checked="" value="EMPRESA" '+
    ...
    {
        tiposHtml = '<input type="radio" onchange="cambiarTipoDeVehiculo(this.value);" value="EMPRESA" '+
                    'name="tipoVehiculo">&nbsp;EMPRESA' + '&nbsp;&nbsp;'+
                    '<input type="radio" onchange="cambiarTipoDeVehiculo(this.value);" checked="" value="SUBCONTRATADO" '+
                    'name="tipoVehiculo">&nbsp;SUBCONTRATADO' + '&nbsp;&nbsp;'+ '';
    }
    else
    {
        tiposHtml = '<input type="radio" onchange="cambiarTipoDeVehiculo(this.value);" value="EMPRESA" name="tipoVehiculo">&nbsp;EMPRESA' + 
                    '&nbsp;&nbsp;'+
                    '<input type="radio" onchange="cambiarTipoDeVehiculo(this.value);" value="SUBCONTRATADO" '+
                    'name="tipoVehiculo">&nbsp;SUBCONTRATADO' + 
                    '&nbsp;&nbsp;'+'';
...
function validarFormulario()
{
    ...
    else if(tipoVehiculo=="SUBCONTRATADO")
    {
        ...
        var dateInicioContrato  = new Date(fechaInicioContrato.value);
        var dateFinContrato     = new Date(fechaFinContrato.value);
        if (dateInicioContrato > dateFinContrato){
            Ext.Msg.alert('Alerta', 'La Fecha de Inicio no puede ser mayor a la Fecha de Fin de Contrato');
            return false;
        }
    }
    else
    {
        Ext.Msg.alert("Alerta","Seleccione el Tipo de Vehículo.");
        return false;
    }

    verificarPlacaExistente('editar');
    return false;
