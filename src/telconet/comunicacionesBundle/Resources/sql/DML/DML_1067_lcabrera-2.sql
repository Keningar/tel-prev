...
TO_CLOB('<html>
  <head>
    ...
    <body> ') || TO_CLOB('
        <div id="bienvenido">ACTA DE ENTREGA DEL SERVICIO O VISITA TÉCNICA </div>
...
') || TO_CLOB('
        ...
        <table id="equipos" width="100%" class="box-section-content" style="font-size:11px; text-align: center; border-collapse: collapse;">
            ...
            {% if elementoCpe %}
            <tr>
                <td style="width:20%" class ="labelGris">{{ elementoCpe.modeloElementoId.tipoElementoId.nombreTipoElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoCpe.modeloElementoId.nombreModeloElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoCpe.modeloElementoId.marcaElementoId.nombreMarcaElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoCpe.serieFisica }}</td>
                <td style="width:20%" class ="labelGris">{{ macCpe.valor }}</td>
            </tr>
            {% endif %}
            {% if elementoOnt %}
            <tr>
                <td style="width:20%" class ="labelGris">{{ elementoOnt.modeloElementoId.tipoElementoId.nombreTipoElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoOnt.modeloElementoId.nombreModeloElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoOnt.modeloElementoId.marcaElementoId.nombreMarcaElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoOnt.serieFisica }}</td>
                <td style="width:20%" class ="labelGris">{{ macOnt.valor }}</td>
            </tr>
            {% endif %}
            {% if elementoWifi %}
            <tr>
                <td style="width:20%" class ="labelGris">{{ elementoWifi.modeloElementoId.tipoElementoId.nombreTipoElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoWifi.modeloElementoId.nombreModeloElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoWifi.modeloElementoId.marcaElementoId.nombreMarcaElemento }}</td>
                <td style="width:20%" class ="labelGris">{{ elementoWifi.serieFisica }}</td>
        ...
        {% endif %}
        <!-- --------------------------------------------------------------------------------------------------------------- -->
        {% if facturacionEquipos == true %}
            <p>FACTURACIÓN DE EQUIPOS</p>
            <div style="font-size:9px;">Se factura el valor de $ {{floatTotalEquipos | number_format(2, ''.'', '''')}} más impuestos por cambio de equipos.
                <ul>
                {% for equipoFacturado in equiposFacturados %}
                    <li> {{ equipoFacturado }}</li>
                {% endfor %}
                </ul> 
            </div>
        {% endif %}
        <!-- --------------------------------------------------------------------------------------------------------------- -->
        <p>VISITA FACTURADA</p>
            {% if facturable == true %}
                <div style="font-size:9px;">Se factura la visita técnica por $ {{ valorFacturable | number_format(2, ''.'', '''')}} más impuestos.</div>
            {% else %}
                <div style="font-size:9px;">NO</div>
            {% endif %}
        <!-- --------------------------------------------------------------------------------------------------------------- -->
') || TO_CLOB('
...
') || TO_CLOB('
        ...
        <div style=" width: 15%; font-size: 9px; float: right;">
            ...
            <br>
            <span>ver-09 | Oct-2018</span>
        </div>
        <div style="clear: both;"></div>
         <!-- --------------------------------------------------------------------------------------------------------------- -->     
    </body>
</html>')
WHERE CODIGO = 'ACT-ENT-MD-VIS'
AND ESTADO = 'Activo';
