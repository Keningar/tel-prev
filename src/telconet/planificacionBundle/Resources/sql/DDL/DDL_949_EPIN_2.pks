create or replace PACKAGE CMKG_CUPOS_CUADRILLAS AS
    /**
...
/
create or replace PACKAGE BODY              CMKG_CUPOS_CUADRILLAS
AS
    PROCEDURE P_SET_CUPOS_CUADRILLAS(Pd_FeInicio     IN DATE,
                                     Pd_FeFin        IN DATE,
                                     Pn_Jurisdiccion IN NUMBER,
    ...
    CURSOR C_CuposTotales(Cd_Fecha VARCHAR2, C_Hora VARCHAR2, Cn_jurisdiccion NUMBER) IS
        SELECT nvl(sum(nvl(cupos_web,0)+nvl(cupos_movil,0)),0)
            ...
            AND to_char(c.hora_desde,'dd/mm/rrrr hh24:mi:ss')= Cd_Fecha||' '||C_Hora;
   ...
   BEGIN
    ...
    --********************************************************
        FOR I IN C_Horarios(to_char(Pd_FeRegistro,'dd/mm/rrrr'), Pn_IdPlantilla)LOOP
            ...
            END IF;
            OPEN C_CuposTotales(to_char(Ld_HoraDesde,'dd/mm/rrrr'), to_char(Ld_HoraDesde,'hh24:mi:ss'),Pn_IdJurisdiccion);
            ...
            WHILE(Ld_HoraDesde<Ld_HoraHasta) LOOP
                ...
                WHILE (Ln_CuposBarrido <= TO_NUMBER((Ln_TotCupo-(nvl(Ln_cupo,0))))) LOOP
