...
class InfoContratoFormaPagoType extends AbstractType
{
    protected $intIdPais;
    ...
    *
    * Se añade parametro para rango usado en años de vencimiento de tarjetas.
    ...
    * Se modifica el anio de vencimiento a mostrar en el HTML de las pantallas
    * ContratoNew y ContratoEdit.
    *
    */
      ...
      $arrayAnios = range(date('Y'), date('Y') + $this->intAnioVencimiento );
        ...
        $builder->add('tipoCuentaId',
                ...
                ->add('bancoTipoCuentaId',
                      'entity',
                      array(
                                'class'       => 'telconet\schemaBundle\Entity\AdmiBancoTipoCuenta',
                                'label'       => '* Banco : ',
                                'label_attr'  => array('class'    => 'campo-obligatorio'),
                                'property'    => 'bancoId',
                                'attr'        => array('class' => 'campo-obligatorio-select','onChange'=>""),
                                'em'          => 'telconet',
                                'required'    => false,
                                'empty_value' => 'Seleccione',
                                'query_builder' => function (EntityRepository $objEntityRepository) use ($intPaisId)
                                {
                                    $objQueryBuilder = $objEntityRepository->createQueryBuilder("t")
                                                                           ->join("t.tipoCuentaId", "tc")
                                                                           ->where("tc.estado = :strEstado")
                                                                           ->andWhere("tc.paisId = :intIdPais");
                                    $objQueryBuilder->setParameter("strEstado",'Activo');
                                    $objQueryBuilder->setParameter("intIdPais",$intPaisId);
                                    return $objQueryBuilder;
                ...
                ->add('numeroCtaTarjeta',
                      'text',
                      array('required'=>false,
                            'label'=>'* N° Tarjeta/Cuenta:',
                            'label_attr' => array('class' => 'campo-obligatorio'),
                            'attr'       => array('maxLength'=>10, 'class' => 'campo-obligatorio'))
                     )
                ->add('titularCuenta',
                      'text',
                      array('label'      => '* Titular:',
                            'required'   => false,
                            'label_attr' => array('class' => 'campo-obligatorio'),
                            'attr'       => array('maxLength'=>90, 'class' => 'campo-obligatorio')
                           )
                     )
                ->add('anioVencimiento', 
                      'choice', 
                      array('choices'     => array_combine($arrayAnios, $arrayAnios),
                            'empty_value' => 'Seleccione...',
                            'empty_data'  => null
                           )
                     )
                ...
                ->add('codigoVerificacion',
                      'text',
                      array('label'      =>'Cod. Verificacion:',
                            'label_attr' => array('class' => ''),
                            'attr'       => array('maxLength'=>4, 'class' => '')
                           )
                     );
    }
