...
class AdmiClausulaContratoRepository extends EntityRepository
{
    public function generarJson($nombre,$estado,$start,$limit)
    {
        ...
        if ($registros) {
            ...
            foreach ($registros as $data)
            {
                ...
                $arr_encontrados[]=array('id_clausula_contrato' =>$data->getId(),
                                         'nombre_clausula' =>trim($data->getNombreClausula()),
                                         'descripcion_tipo_contrato' =>trim($data->getTipoContratoId()->getDescripcionTipoContrato()),
            ...
            if($num == 0)
            {
                $resultado= array('total' => 1 ,
                                 'encontrados' => array('id_clausula_contrato' => 0 , 'nombre_clausula' => 'Ninguno', 'descripcion_tipo_contrato' => 'Ninguno', 'clausula_contrato_id' => 0 , 'clausula_contrato_nombre' => 'Ninguno', 'estado' => 'Ninguno'));
    ...
    public function getRegistros($nombre,$estado,$start,$limit){
        $qb = $this->_em->createQueryBuilder();
            $qb->select('sim')
               ->from('schemaBundle:AdmiClausulaContrato','sim');
            
        $boolBusqueda = false; 
        if($nombre!=""){
            $boolBusqueda = true;
            $qb ->where( 'LOWER(sim.nombreClausula) like LOWER(?1)');
