...
class AdmiTipoNotificacionRepository extends EntityRepository
{
    public function generarJson($nombre,$estado,$start,$limit)
    {
        ...
        if ($registros) {
            ...
            foreach ($registros as $data)
            {
                ...
                $arr_encontrados[]=array('id_tipo_notificacion' =>$data->getId(),
                                         'nombre_tipo_notificacion' =>trim($data->getNombreTipoNotificacion()),
                                         'descripcion_tipo_notificacion' =>trim($data->getDescripcionTipoNotificacion()),
            ...
            if($num == 0)
            {
                $resultado= array('total' => 1 ,
                                 'encontrados' => array('id_tipo_notificacion' => 0 , 'nombre_tipo_notificacion' => 'Ninguno', 'descripcion_tipo_notificacion' => 'Ninguno', 'hipotesis_id' => 0 , 'hipotesis_nombre' => 'Ninguno', 'estado' => 'Ninguno'));
    ...
    public function getRegistros($nombre,$estado,$start,$limit){
        $qb = $this->_em->createQueryBuilder();
            $qb->select('sim')
               ->from('schemaBundle:AdmiTipoNotificacion','sim');
            
        $boolBusqueda = false; 
        if($nombre!=""){
            $boolBusqueda = true;
            $qb ->where( 'LOWER(sim.nombreTipoNotificacion) like LOWER(?1)');
