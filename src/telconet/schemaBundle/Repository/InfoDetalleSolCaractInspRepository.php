...
class InfoDetalleSolCaractInspRepository extends EntityRepository
{
	public function findDetalleSolCaractPorIdDetalleSolicitud($intIdDetalleSolicitud)
    {
		$strQuery = $this->_em->createQuery("SELECT a
		FROM 
			schemaBundle:InfoDetalleSolCaractInsp a, schemaBundle:AdmiTipoSolicitud b
		WHERE 
			a.detalleSolicitudId = ".$intIdDetalleSolicitud." ");
    ...
    /**
      ...
      */     
    public function getSolicitudCaractPorTipoCaracteristica($intIdDetalleSolicitud, $strTipoCaracteristica)
    {                
        $objQuery = $this->_em->createQuery();
        $strDql ="
                        SELECT a                                                
                        FROM                                
                        schemaBundle:InfoDetalleSolCaractInsp a,
        ...
        $objQuery->setParameter('descripcion', $strTipoCaracteristica);
        $objQuery->setParameter('solicitud', $intIdDetalleSolicitud);
        $objQuery->setParameter('estado', "Activo");

        $objQuery->setDQL($strDql);      

        $objDatos = $query->getResult();
    ...
    public function getSolicitudesPorDetSolCaracts($arrayParametros)
    {
        ...
        try
        {
            ...
            if(isset($arrayParametros["strDescripcionSolicitud"]) && $arrayParametros["strDescripcionSolicitud"] == "SOLICITUD INSPECCION")
            {
                ...
                INNER JOIN DB_COMERCIAL.INFO_DETALLE_SOL_CARACT_INSP DET_SOL_CARACT
