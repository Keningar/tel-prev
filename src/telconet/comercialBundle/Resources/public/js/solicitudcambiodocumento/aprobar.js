...
if(sm.getSelection().length > 0)
{
  ...
  for(var i=0 ;  i < sm.getSelection().length ; ++i)
  {
	...
	tipoDoc = tipoDoc + sm.getSelection()[i].data.descuento;  
  ...
  if(estado == 0)
  {
    Ext.Msg.confirm('Alerta','Se aprobarán las solicitudes seleccionadas. Desea continuar?', function(btn){
        if(btn=='yes'){
            Ext.MessageBox.wait("Aprobando solicitudes...");
            Ext.Ajax.request({
                url: url_aprobar,
                method: 'post',
                params: { param : param,tipoDoc : tipoDoc, obs:TFObservacion.getValue()},
        ...
        Ext.Msg.confirm('Alerta','Se rechazaran los servicios seleccionados. Desea continuar?', function(btn){
            if(btn=='yes'){
                ...
                Ext.Ajax.request({
                    url: url_rechazar,
                    method: 'post',
                    params: { param : param, motivoId:motivo_id, obs:TFObservacion.getValue() },
                    success: function(response){
                        Ext.MessageBox.hide();
                        var text = response.responseText;
                        store.load();
