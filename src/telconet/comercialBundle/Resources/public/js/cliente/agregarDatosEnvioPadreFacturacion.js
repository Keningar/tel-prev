...
function winAgregarDatosEnvioPadreFacturacion( data ) {
        ...
        listeners: {
            ...
            afteredit: function(roweditor, changes, record, rowIndex) {
                ...
                for (var i = 1; i < intCountGridDetalle; i++)
                {
                    ...
                    {
                        Ext.Msg.alert('Error', 'Esta numero de telefono ya se encuentra previamente ingresada.');
                        rowEditingTelefono.startEdit(0, 0);
                        break;
                    }
                }
    ...
    if (!winDetalle) {
       ...
       combo_parroquia = new Ext.form.ComboBox({
            ...
            buttons: [
                {
                    ...
                    handler: function() {
                        ...
                        if (arraygridCreaTelefonoDE.getStore().getCount() !== 0)
                        {
                            ...
                            if ( boolTelefono ) 
                            {
                                if ( intCounterTelefono > intNumeroMaxTelefonos )
                                {
                                   Ext.MessageBox.show({
                                       title: 'Alerta',
                                       msg: 'Solo se permite el ingreso de '+ intNumeroMaxTelefonos +' numeros de telefonos.',
                                       buttons: Ext.MessageBox.OK,
                                       icon: Ext.MessageBox.WARNING
                                    });
                                    return false;
                        ...
                        else 
                        {
                            Ext.MessageBox.show({
                                title: 'Alerta',
                                msg: 'Debe ingresar al menos un N° de telefono <b>Telefono</b>.',
                                buttons: Ext.MessageBox.OK,
                                icon: Ext.MessageBox.WARNING
                            });
                            return false;
                        ...
                        if (form1.isValid()) {
                            form1.submit({
                                params: {
                                    ...
                                    datoEnvioMasivo: true
                                },
                                waitMsg: "Procesando",
                                success: function(form1, action) {
                                    Ext.Msg.alert('Success', 'Los datos de envio fueron ingresados con exito');
                                    if (store) {
                                        store.load();
                                    }
                                    form1.reset();
                            ...
                            Ext.Msg.alert('Failed', 'Falta ingresar datos de envio.');
                        }
                    }
                }
            ]
        ...
        winDetalle = Ext.widget('window', {
            title: 'Ingreso de Datos de Envio',
            height: 690,
            width: 650,
            minHeight: 350,
            closable: false,
