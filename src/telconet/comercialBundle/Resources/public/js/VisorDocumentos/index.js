...
Ext.onReady(function() {    
	...
	store = new Ext.data.Store({ 
		...
		[
			...
			{name:'contrato', mapping:'contrato'},		
			      ...
			      [
				      {
					    ...
					    handler: function(grid, rowIndex, colIndex) 
					    {
						    ...
						    if(rec.get('action1')!="icon-invisible")
							    window.location = "/soporte/gestion_documentos/"+rec.get('id')+"/comercial/show";
						    else
				      ...
				      },
				      {
					    ...
					    handler: function(grid, rowIndex, colIndex) 
					    {
						    ...
						    if(rec.get('action4')!="icon-invisible")
							{
								...
								postGuardaHistorial("VISUALIZAR/IMPRIMIR", rec.get('id'));
							}
						    else
							Ext.Msg.alert('Error ','No tiene permisos para realizar esta accion');
					    }
...
function limpiar()
{
    ...
    store.proxy.extraParams = { estado: 'Todos' , modulo: 'COMERCIAL'};
    store.load();
}
