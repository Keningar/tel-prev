   ...
   var arraySolicitudesSeleccionadas = new Array();
    ...
    Ext.define('modelServiciosSeleccionados', {
        ...
        fields: [
            ...
            {name: 'strTipoEnlace', type: 'string'},
            {name: 'strTipoSolicitud', type: 'string'},
            {name: 'strBackupsPendientes', type: 'string'}
        ]
    });
        ...
        [{xtype: 'tbfill'},
            {
                xtype: 'button',
                cls : 'scm-button',
                id: "btnEjecutarSolicitud",
                iconCls: "icon_ejecutar",
                text: '<span class="bold color-green">Ejecutar Seleccionados</span>',
                disabled: true,
                scope: this,
                handler: function () {
                   ...
                   entidadSolicitudMasiva.ejecutarSolicitudes(intIdDetalleSolicitudCab, arraySolicitudesSeleccionadas, storeServiciosSeleccionados);
                }
            },
            {
                xtype: 'button',
    ...
    chkBoxModelSolicitudesSeleccionadas = new Ext.selection.CheckboxModel({
        ...
        renderer : function(val, meta, record, rowIndex, colIndex, store,view){
            ...
            var opcion1 = estadoPrecio == 'N/A' && estadoRadio == 'N/A' && estadoIpccl2 == 'N/A';
            var opcion2 = estadoPrecio == 'Aprobada' && estadoRadio == 'N/A' && estadoIpccl2 == 'N/A';
            var opcion3 = estadoPrecio == 'Aprobada' && estadoRadio == 'Aprobada' && estadoIpccl2 == 'N/A';
            var opcion4 = estadoPrecio == 'Aprobada' && estadoRadio == 'Aprobada' && estadoIpccl2 == 'Aprobada';
                        
            var opcion5 = estadoPrecio == 'N/A' && estadoRadio == 'Aprobada' && estadoIpccl2 == 'N/A';
            var opcion6 = estadoPrecio == 'N/A' && estadoRadio == 'Aprobada' && estadoIpccl2 == 'Aprobada';
            
            var opcion7 = estadoPrecio == 'N/A' && estadoRadio == 'N/A' && estadoIpccl2 == 'Aprobada';
            var opcion8 = estadoPrecio == 'Aprobada' && estadoRadio == 'N/A' && estadoIpccl2 == 'Aprobada';
            
            var boolEstados = opcion1 || opcion2 || opcion3 || opcion4 || opcion5 || opcion6 || opcion7 || opcion8;
            
            if(esServiciosDisponibleAccion(record,true))
            {
                if ((record.get('strEstado') === 'Aprobada' || record.get('strEstado') === 'Fallo') && boolEstados && strArchivo != '') 
                {
        ...
        listeners: {
            selectionchange: function (selectionModel, selected, options) {                
                ...
                gridServiciosSeleccionados.down('#btnEjecutarSolicitud').setDisabled(selected.length == 0);
    ...
    gridServiciosSeleccionados = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
            ...
            {
                header: 'Nivel Aprobación/Estados',
                width: 160,
                align: 'center',                
                ...
                items: [
                    ...
                    },
                    {
                        getClass: function(v, meta, rec) {
                            ...
                            if(esServiciosDisponibleAccion(rec,true))
                            {
                                if ((rec.get('strEstado') === 'Aprobada' || rec.get('strEstado') === 'Fallo') && boolEstados && strArchivo != '') 
                                {
                                    return 'button-grid icon_ejecutar';
                                }
                            }
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            entidadSolicitudMasiva.ejecutarSolicitudes(intIdDetalleSolicitudCab, arraySeleccionado, storeServiciosSeleccionados);
                        }
                    },
                    {
                        getClass: function(v, meta, rec) {
                            ...
                            if(esServiciosDisponibleAccion(rec,false))
                            {
                                if((rec.get('strEstado')==='Aprobada' || rec.get('strEstado')==='Fallo' )&& boolEstados && strArchivo != '')
                                {
...
function esServiciosDisponibleAccion(rec,boolBackupsPdtes)
{
    if(rec.get('strTipoSolicitud') === 'CAMBIO PLAN')
    {
        if(rec.get('strTipoEnlace') === 'BACKUP')
        {
            return false;
        }
        else
        {
            if(rec.get('strBackupsPendientes') === 'S' && boolBackupsPdtes)
            {
                return false;
            }
        }
