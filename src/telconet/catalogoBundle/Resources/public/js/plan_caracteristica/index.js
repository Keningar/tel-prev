Ext.require([
			...
			Ext.define('ListaDetalleModel', {
				...
				fields: [{name:'IdPlan', type: 'string'},
						...
						{name:'linkEditar', type: 'string'},
                                                {name:'linkCaracteristicas', type: 'string'},
						{name:'linkEliminar', type: 'string'},
                                                {name:'linkCondiciones', type: 'string'},
                                                {name:'linkClonar', type: 'string'},
                                                {name:'linkActivar', type: 'string'},
                                                {name:'linkReactivar', type: 'string'}
						]
			}); 
            ...
            function renderAcciones(value, p, record) {
                    ...
                    iconos=iconos+'<b><a href="'+record.data.linkVer+'" onClick="" title="Ver Plan" class="button-grid-show"></a></b>';                                        
                    if(record.data.estado == 'Pendiente')
                    {
                        iconos=iconos+'<b><a href="'+record.data.linkEditar+'" onClick="" title="Editar Plan" class="button-grid-edit"></a></b>';	                    
                    }
                    if(record.data.estado == 'Activo' || record.data.estado == 'Pendiente')
                    {
                        iconos=iconos+'<b><a href="'+record.data.linkCaracteristicas+'" onClick="" title="Agregar Caracteristicas" class="button-grid-agregarCaracteristica"></a></b>';	
                        iconos=iconos+'<b><a href="'+record.data.linkCondiciones+'" onClick="" title="Agregar Condiciones" class="button-grid-agregarCondiciones"></a></b>';	                                        
                    }                    
                    if(record.data.estado == 'Activo')
                    {
                        iconos=iconos+'<b><a href="'+record.data.linkClonar+'" onClick="" title="Clonar Plan" class="button-grid-clonar"></a></b>';	                    
                    }
                    if(record.data.estado == 'Pendiente')
                    {                        
                        iconos=iconos+'<b><a href="#" onClick="activar(\''+record.data.linkActivar+'\')" title="Activar Plan" class="button-grid-aprobar"></a></b>';	                    
                    }                     
                    if(record.data.estado == 'Activo')
                    {
                        iconos=iconos+'<b><a href="#" onClick="eliminar(\''+record.data.linkEliminar+'\')" title="Inactivar Plan" class="button-grid-delete"></a></b>';	                    
                    }
                    if(record.data.estado == 'Inactivo')
                    {                        
                        iconos=iconos+'<b><a href="#" onClick="reactivar(\''+record.data.linkReactivar+'\')" title="Reactivar Plan" class="button-grid-Tuerca"></a></b>';	                    
                    } 
	...
	function limpiar(){
        ...
        function activar(direccion)
	{		
		Ext.Msg.confirm('Alerta','Se Liberara el Plan para su venta. Desea continuar?', function(btn){
			if(btn=='yes'){
				Ext.Ajax.request({
					url: direccion,
					method: 'post',
        ...
        function reactivar(direccion)
	{		
		Ext.Msg.confirm('Alerta','Se Reactivara el Plan para su venta. Desea continuar?', function(btn){
			if(btn=='yes'){
				Ext.Ajax.request({
					url: direccion,
					method: 'post',
