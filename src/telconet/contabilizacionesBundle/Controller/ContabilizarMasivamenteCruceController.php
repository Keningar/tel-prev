...
use telconet\contabilizacionesBundle\Controller\AnticiposCruceController;
use telconet\contabilizacionesBundle\Controller\AnticiposSinClienteCruceController;

class ContabilizarMasivamenteCruceController extends Controller
{
    public function indexAction()
    {
		...
		$resultado=$emfn->getRepository('schemaBundle:InfoPagoCab')->obtenerPagosCruzadosParaMigrarAA();
			...
			foreach($listadoPagos as $pago)
			{
					...
					if($pago->getTipoDocumentoId()->getCodigoTipoDocumento()=='ANT')
					{
					   //Contabilizacion del anticipo		   
						$pagosAntController=new AnticiposCruceController();
					...
					{
					   //Contabilizacion del anticipo		   
						$pagosAntsController=new AnticiposSinClienteCruceController();
