...
class PagosAnulacionController extends Controller
{
    ...
    public function contabilizarPagosAction($id_pago)
    {
		/*
		 * Proceso:
		...
		$info_total = $emfinan->getRepository('schemaBundle:InfoPagoCab')->getDetallePago($id_pago,"PAG");
			
		
		//die();
		//echo "cta contable obtenida!";
		//die;
...
//				$oci_con = oci_connect("TNET_SISTEMAS", "TNETSISTEMAS", "172.24.5.78/dboracle");
				...
				foreach($info_total as $info)
				{
					...
					else
					{
						...
						$cta_contable_cliente = $emfinan->getRepository('schemaBundle:InfoPagoCab')
						->getCtaCliente($empresa_id, $oficina_id,"PAG");
						...
						$no_cta="";
						$es_tarjeta="N";
						//echo "<br>Banco obtenido:".$info['bancoTipoCuentaId'];
						//die;
						if ($info['codigoFormaPago']!='EFEC' && $info['codigoFormaPago']!='CHEQ' && $info['codigoFormaPago']!='RF2' && $info['codigoFormaPago']!='RF8' && $info['codigoFormaPago']!='RTF')
						{
						...
						//echo "entro por falso";die;
							...
							{
								$cta_contable_array=$this->verificarClasificacion($info['codigoFormaPago'],$empresa_id, $oficina_id);
								//echo "ya verifico clasificacion";die;
								$cta_contable=$cta_contable_array['ctaContablePagos'];
								$no_cta=$cta_contable_array['noCta'];
								if($info['bancoTipoCuentaId']!="")
								{
						...
						if($no_cta!="" && $es_tarjeta=="N")
						{
							...
							//$migra_arckmm->setComentario(substr('Pago user: '.$ptocliente['login'].' TELCOS '.date('d-m-Y').' '.$nombre_oficina,0,250));
							$nombre_banco_str="";
							$glosa="";
							...
							$glosa=substr('Anul. Pago. user: '.$glosa,0,250);
							
							/*$migra_arckmm->setComentario(substr('Pago. user: '.$glosa,0,250));
							...
							//$migra_arckmm->setFechaCreacion(new \DateTime(date('d-m-Y')));					
							...
							//$migra_arckml->setAno(date('Y'));
							$migra_arckml->setAno($fecha_exp[0]);
							//$migra_arckml->setMes(date('n'));
							$migra_arckml->setMes($fecha_exp[1]);
							$migra_arckml->setMontoDc($info['valorPago']);
							//$migra_arckml->setGlosa(substr('NC:0- pago user: '.$ptocliente['login'].' TELCOS '.date('d-m-Y').' '.$nombre_oficina,0,100)); 
							//$migra_arckml->setGlosa(substr('NC:0- pago user: '.$glosa,0,100)); 
							$migra_arckml->setGlosa(substr('Pago. user: '.$glosa,0,100)); 
						...
						else
						{
							...
							//$migra_arcgae->setNoAsiento($id_pago.'77');
							...
							*/
							...
							$glosa=substr('Pago. user: '.$glosa,0,250);
							
							/*
							//$migra_arcgae->setDescri1(substr('Pago. user: '.$ptocliente['login'].' TELCOS '.$fecha_mod.' '.$nombre_oficina,0,250));
							$migra_arcgae->setDescri1(substr('Pago. user: '.$glosa,0,250));
							...
							//$migra_arcgal->setNoAsiento($id_pago.'77');
							$migra_arcgal->setNoAsiento($info['id'].'77');
							$migra_arcgal->setNoLinea('1');
							$migra_arcgal->setCuenta($cta_contable);
							//$migra_arcgal->setDescri('Pago. user: '.$ptocliente['login'].' TELCOS '.$fecha_mod.' '.$nombre_oficina);
							$migra_arcgal->setDescri('Pago. user: '.$glosa);
							//Codigo de diario nuevo que debe crear luis
							if($info['codigoFormaPago']=='RF2' || $info['codigoFormaPago']=='RF8' || $info['codigoFormaPago']=='RTF')
							...
							$migra_arcgal->setCuenta($cta_contable_cliente['ctaContableClientes']);
							//$migra_arcgal->setDescri('Pago. user: '.$ptocliente['login'].' TELCOS '.$fecha_mod.' '.$nombre_oficina);    
							$migra_arcgal->setDescri('Pago. user: '.$glosa);            
							//Codigo de diario nuevo que debe crear luis
							if($info['codigoFormaPago']=='RF2' || $info['codigoFormaPago']=='RF8' || $info['codigoFormaPago']=='RTF')
	...
	public function verificarClasificacion($codigoFormaPago,$empresa_id, $oficina_id)
	{
		...
		if($codigoFormaPago=='RF2' || $codigoFormaPago=='RF8' || $codigoFormaPago=='RTF' )
		{
			$em = $this->getDoctrine()->getManager("telconet");
			//Debo cargar las ctas contables de las retenciones de la empresa
			$cta_contable=$em->getRepository('schemaBundle:AdmiFormaPago')->getCtaRetenciones();
			
			return $cta_contable;
		}
		
		if($codigoFormaPago=='TARC')
		{
			$em = $this->getDoctrine()->getManager("telconet");
			//Debo cargar las ctas contables de las retenciones de la empresa
			$cta_contable=$em->getRepository('schemaBundle:InfoPagoCab')->getCtaTarjeta();
	...
	public function buscarExistentente($id_pago,$id_pago_det,$empresa_id)
	{
		...
		$resultado_dos = $em_naf->getRepository('schemaBundle:MigraArcgae')->getExisteAnuladosContabilidad($id_pago_det,$empresa_id);
